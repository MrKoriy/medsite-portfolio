...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    // Seamless handoff: when scrolling past the scroll-world track into page content,
    // fade out the fixed stage & UI so standard sections are clear and unobstructed.
    const worldEnd = totalW * vh;
    if (y > worldEnd) {
      const exitFade = clamp(1 - (y - worldEnd) / (0.5 * vh));
      stage.style.opacity = exitFade;
      copylayer.style.opacity = exitFade;
      route.style.opacity = exitFade;
      topbar.style.opacity = exitFade;
      stage.style.pointerEvents = exitFade > 0.01 ? 'none' : 'none';
